...
export const Modal = ({ isOpen = true, onClose, title, icon: Icon, iconColor = 'text-accent', size = 'md', zIndex = 'z-50', children }) => {
    ...
    return (
        <section className={`fixed inset-0 bg-overlay backdrop-blur-xs w-full h-full flex flex-col items-center justify-center ${zIndex} max-lg:p-2 p-4`}>
            <section
                className={`bg-surface rounded-xl border border-outline w-full ${sizeMap[size]} relative max-lg:max-h-[calc(100dvh-1rem)] max-h-[calc(100dvh-2rem)] overflow-y-auto scrollbar-none max-lg:pb-0 pb-0`}
